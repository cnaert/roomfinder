...
def process_webhook():
    ...
    #print "demo_room_id: ", demo_room_id
...
def process_demoroom_message(post_data):
    ...
    demo_room_id = post_data["data"]["roomId"]
    print "message_id", message_id
    message = get_message(message_id)
    print "personEmail: ", message["personEmail"] 
    print "bot_email: ", bot_email
    ...
    if text.lower().find("dispo") > -1 or text.lower().find("available") > -1:
        ...
        # print start[6:]
        # print "2017-01-30T02:12:12"
        # start2 = datetime.datetime.strptime(str(start[6:]), "%Y-%m-%dT%H:%M:%S")
        # print start2.strftime("%A, %d. %B %Y %I:%M%p")
        reply = ""
        toto=list(results)
        ...
        if number:
            if len(number) == 1:
                inf = int(number[0])
                filtered_results=[result for result in toto if int(result[5])>=inf]
                sys.stderr.write("filtered_results: "+str(filtered_results)+"\n")
                reply += ", with more than "+str(inf)+" seats,"
            else:
                inf = int(number[0])
                sup = int(number[1])
                filtered_results=[result for result in toto if int(result[5])>=inf and int(result[5])<=sup]
                sys.stderr.write("filtered_results: "+str(filtered_results)+"\n")
                reply += ", with more than "+str(inf)+" and less than "+str(sup)+" seats,"
        else:
        ...
        if titi:
            filtered_results_with_headers = [["Status", "Name", "Email", "Floor", "Zone", "Seats", "Description"]]
            filtered_results_with_headers.extend(titi)
            lenMax = [0,0,0,0,0,0,0]
            reply = "The current available rooms"+reply+" are:\n```\n"
            for result in filtered_results_with_headers:
                for element in range(0,len(result)):
                    if lenMax[element] < len(result[element])+2:
                        lenMax[element] = len(result[element])+2
            print lenMax
            for result in filtered_results_with_headers:
                for element in range(0,len(result)):
                    #filter to remove email column --> Too many characters!
                    if element != 2:
                        if element == 0:
                            reply += result[element]
                        else: 
                            reply += "|" + result[element]
                        for i in range(1,lenMax[element]-len(result[element])):
                            reply += " " 
                reply += "\n"
            reply += "```"
            message_type = "rich"
    ...
    # Check if message contains phrase "help" and display generic help message
        ...
        # Find the floor
        keyword_list = re.findall(r'PAR-[0-7]', text) + re.findall(r'[0-7]', text)
        print "keyword_list= "+str(keyword_list)
        keyword_list.reverse()
        floor=keyword_list.pop()
        file = display_map(floor)
        print "display_map: "+floor
        message_type="image"
    elif text.lower().find("book") > -1 or text.lower().find("reserve") > -1 :
        # Find the room name
        keyword_list = re.findall(r'[\S-]+', text)
        ...
        if reply == 200:
            reply = "The room " + keyword + " is booked for you. Check your emails.. :)"
            message_type="image"
            file = display_mapRoom(keyword)
            #send_message_to_room(demo_room_id, reply2,"image")
        else: reply = "Sorry, " + str(reply)
    ...
    send_message_to_room(demo_room_id, reply, file, message_type)
...
def send_message_to_queue(message):
    u = app_server + "/book"
    r = requests.post(u, headers=app_headers, data=message )
...
def book_room(room_name,user_email,user_name):
    ...
    start, end, results = get_available()
    dispo_list=[r[1].upper() for r in results]
    if room_name in dispo_list:
        ...
        room_email = [mail for (name, mail) in results if name == room_name]
        data = {"cmd":"book", 
                "data": {"starttime": starttime, "endtime": endtime, "user_name": user_name, "user_email": user_email, "room_name": room_name, "room_email": room_email}
            }
...
def display_map(floor):
    ...
    t=re.search(r'PAR-[0-7]',floor)
    if t is not None:
        return picture_server + "/map/img/FR/PAR/"+t.group(0)+".png"
    else:
        t=re.search(r'[0-7]',floor)
        if t is not None:
            return picture_server + "/map/img/FR/PAR/PAR-"+t.group(0)+".png"
        else:
            return "Floor "+ floor + " not known"

def display_mapRoom(RoomURI):
    sys.stderr.write("Display map of the room: "+RoomURI+"\n")
    return picture_server + "/map/img/Rooms/"+RoomURI+".png"
...
def send_message_to_room(room_id, message, file, message_type):
    spark_u = spark_host + "v1/messages"
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "-p", "--picUrl", help="Address of picture server", required=False
    )
    parser.add_argument(
    ...
    picture_server = args.picUrl
    if (picture_server == None):
        picture_server = os.getenv("roomfinder_picture_server")
        if (picture_server == None):
            picture_server = raw_input("What is the picture server address? ")
    # print "Picture server: " + photo_server
    sys.stderr.write("Picture server: " + picture_server + "\n")
